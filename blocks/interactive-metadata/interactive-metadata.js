...
let createTag;
let loadStyle;
let syncPausePlayIcon;
let applyAccessibilityEvents;
let addAccessibilityControl;
...
function defineDeviceByScreenSize() {
  ...
  if (innerWidth >= DESKTOP_SIZE) return 'DESKTOP';
  if (innerWidth <= MOBILE_SIZE) return 'MOBILE';
  return 'TABLET';
}
...
export async function handleImageTransition(stepInfo, transitionCfg = {}) {
  ...
  if (displayPics.length) {
    ...
    if (displayVideos[vidIdx].nodeName === 'A') {
      const posterImg = displayVideos[vidIdx].getAttribute('data-video-poster') ?? '';
      ...
      const posterImg = displayVideos[vidIdx].getAttribute('poster') ?? '';
...
async function getTargetArea(el) {
  ...
  const intEnb = metadataSec.querySelector('.marquee, .aside, .media');
...
export default async function init(el) {
  ([{ createTag, loadStyle, createIntersectionObserver }, {
    syncPausePlayIcon,
    applyAccessibilityEvents,
    addAccessibilityControl,
  }] = await Promise.all([
