...
const {
  ...
  isDarkMode,
  loadStyles,
} = await import(`${LIBS}/blocks/global-navigation/utilities/utilities.js`);
...
class Footer {
    ...
    if (url.hash !== '' && url.hash !== '#_dnt') {
      ...
      const loadRegionNav = async () => {
        ...
        if (block && getConfig().standaloneGnav) {
          ...
          const { default: initRegionNav } = await import(`${LIBS}/blocks/region-nav/region-nav.js`);
          initRegionNav(block);
      ...
      const { default: initFragment } = await import(`${LIBS}/blocks/fragment/fragment.js`);
  ...
  processJarvisLink = async () => {
    ...
    const { getMetadata: sectionMetadata } = await import(`${LIBS}/blocks/section-metadata/section-metadata.js`);
    ...
    if (Object.keys(jarvisMeta).length) {
      ...
      const { initJarvisChat } = await import(`${LIBS}/features/jarvis-chat.js`);
