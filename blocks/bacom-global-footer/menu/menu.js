...
const { getConfig, shouldBlockFreeTrialLinks } = await import(`${LIBS}/utils/utils.js`);
...
const isDesktopForContext = (context = 'viewport') => {
  const isContainerResponsiveFooter = document.querySelector('.bacom-global-footer')?.classList.contains('responsive-container');
  if (context === 'footer' && isContainerResponsiveFooter) {
    const footerElement = document.querySelector('footer.bacom-global-footer');
