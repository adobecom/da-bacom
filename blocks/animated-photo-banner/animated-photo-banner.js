...
function setElementTransform(element, xPosition, yPosition, scale = CONFIG.DEFAULT_SCALE, blur = false) {
  element.style.transform = `translate(calc(${xPosition}vw - 50%), calc(${yPosition}rem - 50%)) scale(calc(${scale}))`;
  if (blur) {
    element.animate(
      [{ filter: 'blur(50px)' }, { filter: 'blur(0px)' }],
      {
        duration: 700,
        easing: 'ease-out',
        fill: 'forwards',
...
function groupImagesByWave(images, paramsList) {
  ...
  images.forEach((image, index) => {
    if (index === 0) image.style.zIndex = 2;
...
function showHeader(header, headerParams) {
  ...
  // Header is always visible - just ensure it's shown
  setElementVisibility(header, true, 0);
}
...
function createAnimationSequence(container, images, paramsList, headerParams) {
  ...
  // Group images
  const { waveGroups } = groupImagesByWave(images, paramsList);
