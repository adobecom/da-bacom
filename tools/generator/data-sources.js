...
function dispatchError(type, message) {
  const error = new CustomEvent(`${type}Error`, { detail: { message } });
...
async function getCaasCollections(context, token) {
  ...
  try {
    ...
    dispatchError('caasOptions', error.message);
    return { contentTypes: [], primaryProducts: [] };
  }
}
...
async function getMarketoPOI() {
  try {
    ...
    dispatchError('mktoOptions', error.message);
    return [];
  }
}
